...
class YouTubeDownloader:
    ...
    def create_widgets(self):
        ...
        header = ctk.CTkLabel(main_frame, text=self._("title"), font=("Arial", 20, "bold"))
        ...
        ctk.CTkLabel(url_frame, text=self._("url_label"), font=("Arial", 12)).pack(side="left", padx=(0, 10))

        self.url_entry = ctk.CTkEntry(url_frame, width=300, placeholder_text="Enter YouTube URL")
        ...
        self.paste_btn = ctk.CTkButton(url_frame, text=self._("paste_button"), command=self.safe_paste_and_fetch, width=80)
        ...
        ctk.CTkLabel(info_frame, text=self._("video_title"), font=("Arial", 12)).pack(side="left", padx=(0, 10))
        ...
        ctk.CTkLabel(self.format_frame, text=self._("format_label"), font=("Arial", 12)).grid(row=0, column=0, sticky="w", padx=5, pady=5)
        self.format_options = ctk.CTkOptionMenu(self.format_frame, variable=self.download_format, values=["mp4", "mp3"], command=self.update_quality_options)
        self.format_options.grid(row=0, column=1, sticky="w", padx=5, pady=5)

        self.quality_frame = ctk.CTkFrame(self.format_frame)
        self.quality_frame.grid(row=1, column=0, columnspan=2, sticky="w", padx=5, pady=5)
        ...
        self.download_btn = ctk.CTkButton(self.button_frame, text=self._("download_button"), command=self.download_video)
        self.download_btn.grid(row=0, column=0, padx=5)

        self.reset_btn = ctk.CTkButton(self.button_frame, text=self._("reset_button"), command=self.reset_fields)
        self.reset_btn.grid(row=0, column=1, padx=5)

        self.exit_btn = ctk.CTkButton(self.button_frame, text=self._("exit_button"), command=self.root.quit)
        self.exit_btn.grid(row=0, column=2, padx=5)

        self.open_folder_btn = ctk.CTkButton(self.button_frame, text=self._("open_folder_button"), command=self.open_download_folder, state="disabled")
        self.open_folder_btn.grid(row=0, column=3, padx=5)

        self.status_var = ctk.StringVar(value=self._("ready"))
        self.status_bar = ctk.CTkLabel(main_frame, textvariable=self.status_var, anchor="w")
        self.status_bar.pack(fill="x", pady=(10, 0))

        self.url_entry.focus()
    
    def setup_menus(self):
        menubar = Menu(self.root)  # Sử dụng tkinter.Menu thay vì ctk.CTkMenu
    ...
    def update_quality_options(self, event=None):
        ...
        if format_type == "mp4":
            ctk.CTkLabel(self.quality_frame, text=self._("quality_label"), font=("Arial", 12)).pack(side="left", padx=(0, 10))
            quality_options = ctk.CTkOptionMenu(self.quality_frame, variable=self.quality, values=["1080p", "720p", "480p"])
            quality_options.pack(side="left")
            self.quality.set("720p")
        else:
            ctk.CTkLabel(self.quality_frame, text=self._("bitrate_label"), font=("Arial", 12)).pack(side="left", padx=(0, 10))
            bitrate_options = ctk.CTkOptionMenu(self.quality_frame, variable=self.bitrate, values=["64", "96", "128", "160", "192", "256", "320"])
